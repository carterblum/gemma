...
from kauldron.typing import (  # pylint: disable=g-multiple-import,g-importing-member
    Array,
    Float,
    Int,
    PRNGKey,
...
from gemma.gm.text import _prefill, _sampler_loop, _sampling, _tokenizer
...
class Sampler:
    ...
    * The BOS (beginning of sequence) token is automatically added.
    ...
    def _normalize_tokens(self, tokens: Sequence[str | int] | None) -> tuple[int, ...]:
        if tokens is None:
            return ()
        else:
