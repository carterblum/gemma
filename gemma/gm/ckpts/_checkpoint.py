...
from gemma.gm.ckpts import _compat, _quantization
...
class LoadCheckpoint(kd.ckpts.AbstractPartialLoader):
    ...
    def transform(
        ...
        new_params = load_params(self.path, params=state.params, quantize=self.quantize)
...
def _flat_to_nested(params: Params) -> Params:
    ...
    transformer_params = _flat_to_nested_single(transformer_params, name="transformer")
...
def _is_flat_layout(params: Params) -> bool:
    ...
    return all(
        k.startswith(("transformer/", "SigLiPFromPatches_0/")) for k in params.keys()
    )
