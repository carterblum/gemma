...
from grox.core import Sites, tag
...
class Embedder(nn.Module):
    ...
    def encode(self, x: jax.Array) -> jax.Array:
        ...
        Returns:
          ...
          embed_dim].
        """
        input_embedding_table = tag(Sites.W_EMBED, self.input_embedding_table)
        x = input_embedding_table[(x,)]
...
class Attention(nn.Module):
    ...
    def __call__(
        ...
        query_proj = tag(Sites.RAW_Q(self.layer), query_proj)
        key_proj = tag(Sites.RAW_K(self.layer), key_proj)
        value_proj = tag(Sites.RAW_V(self.layer), value_proj)
        ...
        logits = tag(Sites.RAW_ATTN_LOGITS, logits)
        ...
        padded_logits = tag(Sites.ATTN_LOGITS(self.layer), padded_logits)
...
class Block(nn.Module):
    ...
    def __call__(
        ...
        outputs = tag(Sites.RESID_POST_NORM(self.layer), outputs)
